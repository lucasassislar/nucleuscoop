...
namespace Nucleus
{
    public static class CmdUtil
    {
        ...
        public static void LinkDirectory(string root, DirectoryInfo currentDir, string destination, out int exitCode, string[] dirExclusions, string[] fileExclusions, bool overrideSpecial = false)
        {
            exitCode = 1;
            ...
            if (special)
            {
                ...
                CmdUtil.LinkFiles(currentDir.FullName, destination, out exitCode, fileExclusions);

                DirectoryInfo[] children = currentDir.GetDirectories();
                for (int i = 0; i < children.Length; i++)
                {
                    DirectoryInfo child = children[i];
                    LinkDirectory(root, child, Path.Combine(destination, child.Name), out exitCode, dirExclusions, fileExclusions);
                }
            }
            else
            {
